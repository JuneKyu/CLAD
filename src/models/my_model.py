...
from .cluster_models import GaussianMixture_clustering
...
class Model(object):
    ...
    def __init__(self, dataset, cluster_num, cluster_type, classifier_type, use_noise_labeling):
        """TODO: to be defined. """
    ...
    def classify(self): # classify with cluster model
        ...
        # to store classifiers for when each cluster set to normal
        #  classifiers = []
        test_predict_list = []
        # num of clusters
        n_clusters = 0
        ...
        # TODO -> train_pred_label -- extracted from clustering model
        for cluster_index in range(len(np.unique(self.train_clusters))):
            ...
            train_cluster_labels = (self.train_clusters == cluster_index).astype(int)
            val_cluster_labels = (self.val_clusters == cluster_index).astype(int)
            test_cluster_labels = (self.test_clusters == cluster_index).astype(int)
        ...
        for combi_num in range(2, self.cluster_num + 1): # ex : 2, 3, 4, 5
            ...
            for combi_ in range(len(data_combi_list)):
                ...
                for j in range(1, len(data_combi_list[combi_])):
                    ...
                    print("cluster_num - {}, combination - {}, f1 - {:.4f}, threshold - {}"\
                            .format(self.cluster_num, combi_list[combi_], f1, j))
                    log.info("cluster_num - {}, combination - {}, f1 - {:.4f}, threshold - {}"\
                            .format(self.cluster_num, combi_list[combi_], f1, j))
